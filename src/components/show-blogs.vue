...
<template>
    <div id="show-blogs">
        ...
        <div v-for="blog in filteredBlogs" class="single-blog">
            <router-link v-bind:to="'/blog/' + blog.id"><h2 v-rainbow>{{ $filters.buyutFiltre(blog.title) }}</h2></router-link>
            <p>{{ blog.content }}</p>
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        created() {
            self = this;
            fetch("https://myvueapp-73910-default-rtdb.firebaseio.com/posts.json", {
                method: 'GET',
                header: {
